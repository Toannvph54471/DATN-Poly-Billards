...
@section('content')
        ...
        <div class="stat-card p-4">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-xl font-bold text-gray-800">{{ $tables->whereIn('status', ['occupied', 'quick'])->count() }}</p>
                </div>
                ...
                <div class="space-y-2 max-h-60 overflow-y-auto">
                    @foreach($expiredTables as $item)
                        <div class="border-b pb-2 last:border-b-0">
                            <div class="font-medium">{{ $item['table']->table_name }} (#{{ $item['table']->table_number }})</div>
                            <div class="text-sm text-gray-600">Đã quá: <span class="font-bold text-red-600">{{ $item['over'] }} phút</span></div>
            ...
            <div class="absolute hidden group-hover:block bg-white shadow-lg rounded-lg p-4 mt-2 z-50 w-80">
                ...
                <div class="space-y-2 max-h-60 overflow-y-auto">
                    @foreach($criticalTables as $item)
                        <div class="border-b pb-2 last:border-b-0">
                            <div class="font-medium">{{ $item['table']->table_name }} (#{{ $item['table']->table_number }})</div>
                            <div class="text-sm text-gray-600">Còn lại: <span class="font-bold text-orange-600">{{ $item['min'] }} phút</span></div>
                            <a href="{{ route('admin.tables.detail', $item['table']->id) }}" 
                               class="text-blue-600 hover:text-blue-800 text-xs inline-block mt-1">
                                Xem chi tiết →
                            </a>
            ...
            <div class="absolute hidden group-hover:block bg-white shadow-lg rounded-lg p-4 mt-2 z-50 w-80">
                ...
                <div class="space-y-2 max-h-60 overflow-y-auto">
                    @foreach($warningTables as $item)
                        <div class="border-b pb-2 last:border-b-0">
                            <div class="font-medium">{{ $item['table']->table_name }} (#{{ $item['table']->table_number }})</div>
                            <div class="text-sm text-gray-600">Còn lại: <span class="font-bold text-yellow-600">{{ $item['min'] }} phút</span></div>
                            <a href="{{ route('admin.tables.detail', $item['table']->id) }}" 
                               class="text-blue-600 hover:text-blue-800 text-xs inline-block mt-1">
                                Xem chi tiết →
                            </a>
    ...
    <form method="GET" action="{{ route('admin.tables.index') }}" class="mb-6 bg-white p-4 rounded-xl shadow-sm">
        ...
        <div class="mt-3 flex space-x-2">
            ...
            <a href="{{ route('admin.tables.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600">
                Reset
            </a>
        </div>
    </form>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($tables as $table)
                        ...
                        <tr class="hover:bg-gray-50 transition 
                            @if($isExpired) bg-red-50 
                            @elseif($isCritical) bg-orange-50 
                            @elseif($isWarning) bg-yellow-50 @endif">
                            <td class="px-6 py-4 relative group">
                                <!-- Clickable row for table detail -->
                                <a href="{{ route('admin.tables.detail', $table->id) }}" class="block hover:text-blue-600 transition-colors">
                                ...
                                <!-- Hover details for combo warnings -->
                                @if($isExpired || $isCritical || $isWarning)
                                    <div class="absolute hidden group-hover:block bg-white shadow-lg rounded-lg p-3 mt-1 z-50 w-64 left-0 border border-gray-200">
                                        <div class="font-bold text-gray-800 mb-1">Thông tin Combo:</div>
                                        <div class="text-sm">
                                            <div class="flex justify-between mb-1">
                                                <span class="text-gray-600">Bàn:</span>
                                                <span class="font-medium">{{ $table->table_name }} (#{{ $table->table_number }})</span>
                                            </div>
                                            @if($isExpired)
                                                <div class="flex justify-between mb-1">
                                                    <span class="text-gray-600">Trạng thái:</span>
                                                    <span class="font-bold text-red-600">HẾT GIỜ COMBO</span>
                                                </div>
                                                <div class="flex justify-between mb-1">
                                                    <span class="text-gray-600">Đã quá:</span>
                                                    <span class="font-medium text-red-600">{{ abs($remain) }} phút</span>
                                                </div>
                                            @else
                                                <div class="flex justify-between mb-1">
                                                    <span class="text-gray-600">Trạng thái:</span>
                                                    <span class="font-bold 
                                                        @if($isCritical) text-orange-600 
                                                        @else text-yellow-600 @endif">
                                                        @if($isCritical) CẢNH BÁO KHẨN
                                                        @else CẢNH BÁO @endif
                                                    </span>
                                                </div>
                                                <div class="flex justify-between mb-1">
                                                    <span class="text-gray-600">Thời gian còn:</span>
                                            ...
                                            <div class="mt-2 pt-2 border-t border-gray-100">
                                                <a href="{{ route('admin.tables.detail', $table->id) }}" 
                                                   class="text-blue-600 hover:text-blue-800 text-xs font-medium block text-center hover:underline">
                                                    <i class="fas fa-external-link-alt mr-1"></i>Xem & Xử lý ngay
                                                </a>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="flex flex-col space-y-1">
                                    <span class="px-2 py-1 text-xs rounded-full 
                                        {{ $table->status === 'available' ? 'bg-green-100 text-green-800' : 
                                           ($table->status === 'occupied' || $table->status === 'quick' ? 'bg-red-100 text-red-800' :
                                           ($table->status === 'paused' ? 'bg-yellow-100 text-yellow-800' : 
                            ...
                            <td class="px-6 py-4 text-sm font-medium">{{ number_format($table->getHourlyRate()) }} đ/giờ</td>
                            <td class="px-6 py-4">
                                @if ($isCombo && $hasTime && $remain !== null)
                                    <div class="relative group/combo">
                                        <a href="{{ route('admin.tables.detail', $table->id) }}" 
                                           class="inline-block hover:opacity-90 transition-opacity">
                                            <span class="px-2 py-1 text-xs rounded-full cursor-pointer
                                                {{ $isExpired ? 'bg-red-100 text-red-800 hover:bg-red-200' : 
                                                   ($isCritical ? 'bg-red-100 text-red-800 hover:bg-red-200' :
                                                   ($isWarning ? 'bg-yellow-100 text-yellow-800 hover:bg-yellow-200' : 
                                                   'bg-green-100 text-green-800 hover:bg-green-200')) }}">
                                                @if ($isExpired)
                                                    <i class="fas fa-exclamation-circle mr-1"></i>Hết giờ (-{{ abs($remain) }}')
                                                @else
                                        ...
                                        <!-- Combo hover tooltip -->
                                        <div class="absolute hidden group-hover/combo:block bg-white shadow-lg rounded-lg p-3 mt-1 z-50 w-56 left-1/2 transform -translate-x-1/2 border border-gray-200">
                                            <div class="font-bold text-gray-800 mb-1">Thông tin Combo</div>
                                            <div class="text-sm text-gray-600 space-y-1">
                                                ...
                                                <div class="flex justify-between">
                                                    ...
                                                    <span class="font-bold 
                                                        ...
                                                        @elseif($isCritical) text-orange-600 
                                                        @elseif($isWarning) text-yellow-600 
                                                        @else text-green-600 @endif">
                                                        @if($isExpired) HẾT GIỜ
                                                        @elseif($isCritical) CẤP BÁO
                                                        @elseif($isWarning) CẢNH BÁO
                                                        @else BÌNH THƯỜNG @endif
                                                    </span>
                                                </div>
                                                <div class="flex justify-between">
                                                    <span>Thời gian:</span>
                                                ...
                                                <div class="mt-2 pt-2 border-t border-gray-100">
                                                    <a href="{{ route('admin.tables.detail', $table->id) }}" 
                                                       class="text-blue-600 hover:text-blue-800 text-xs font-medium text-center block hover:underline">
                                ...
                                @elseif($isProductsOnly)
                                    <div class="relative group/combo">
                                        <a href="{{ route('admin.tables.detail', $table->id) }}" 
                                           class="inline-block hover:opacity-90 transition-opacity">
                                            <span class="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full cursor-pointer hover:bg-blue-200">
                                                <i class="fas fa-shopping-basket mr-1"></i>Combo sản phẩm
                                            </span>
                                        </a>
                                        
                                        <!-- Products combo hover tooltip -->
                                        <div class="absolute hidden group-hover/combo:block bg-white shadow-lg rounded-lg p-3 mt-1 z-50 w-56 left-1/2 transform -translate-x-1/2 border border-gray-200">
                                            <div class="font-bold text-gray-800 mb-1">Combo Sản Phẩm</div>
                                            <div class="text-sm text-gray-600">
                                                <div class="flex justify-between mb-1">
                                                    <span>Bàn:</span>
                                                    <span class="font-medium">#{{ $table->table_number }}</span>
                                                </div>
                                                <div class="mb-1">Gồm các sản phẩm dịch vụ</div>
                                                <div class="mt-2 pt-2 border-t border-gray-100">
                                                    <a href="{{ route('admin.tables.detail', $table->id) }}" 
                                                       class="text-blue-600 hover:text-blue-800 text-xs font-medium text-center block hover:underline">
                                                        <i class="fas fa-external-link-alt mr-1"></i>Xem chi tiết
                            ...
                            <td class="px-6 py-4 text-right space-x-3">
                                ...
                                @if ($isExpired)
                                    <button type="button" 
                                            onclick="handleExpiredCombo({{ $table->id }})" 
                                            class="text-red-600 hover:text-red-800 hover:underline" title="Xử lý combo hết giờ">
