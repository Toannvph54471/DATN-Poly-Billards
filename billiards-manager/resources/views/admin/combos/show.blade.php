        ...
        <div class="bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="space-y-3">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Giá bán</label>
                        <p class="text-lg font-semibold text-green-600">{{ number_format($actualValue, 0, ',', '.') }}₫</p>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <span
                        class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $combo->isActive() ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
            ...
            <table class="w-full">
                <thead>
                    <tr class="bg-gray-50 border-b border-gray-200">
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Đơn giá
                        </th>
                    ...
                    @foreach ($combo->comboItems as $index => $item)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    <div
                                        class="flex-shrink-0 h-10 w-10 bg-gray-200 rounded-lg flex items-center justify-center mr-3">
                                        <i class="fas fa-cube text-gray-500"></i>
                                    </div>
                                    <div>
                                        <div class="text-sm font-medium text-gray-900">
                                            {{ $item->product->name ?? 'Không xác định' }}</div>
                                        <div class="text-sm text-gray-500">{{ $item->product->product_code ?? 'N/A' }}
                                        </div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">
                                {{ number_format($item->getTotalPrice(), 0, ',', '.') }}₫</td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot class="bg-gray-50">
                    <tr>
                        <td colspan="4" class="px-6 py-4 text-right text-sm font-medium text-gray-900">Tổng giá trị
                            combo:</td>
