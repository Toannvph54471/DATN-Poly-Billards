...
@section('content')
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Chỉnh sửa Combo</h1>
            <p class="text-gray-600">Cập nhật thông tin combo</p>
        </div>
        ...
        <form action="{{ route('admin.combos.update', $combo->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="combo_code" id="combo_code" value="{{ old('combo_code', $combo->combo_code) }}"
                ...
                <div>
                    ...
                    <input type="number" name="actual_value" id="actual_value" value="{{ old('actual_value', $combo->actual_value) }}"
                        class="w-full bg-gray-50 border border-gray-300 rounded-lg px-4 py-3 text-gray-800 focus:ring-2 focus:ring-blue-500"
                ...
                <div>
                    ...
                    <select name="status" id="status"
                        ...
                        <option value="Inactive" {{ old('status', $combo->status) == 'Inactive' ? 'selected' : '' }}>Ngừng hoạt động</option>
                    </select>
                    @error('status')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="w-full bg-gray-50 border border-gray-300 rounded-lg px-4 py-3 text-gray-800 focus:ring-2 focus:ring-blue-500"
                        placeholder="Nhập mô tả combo">{{ old('description', $combo->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
