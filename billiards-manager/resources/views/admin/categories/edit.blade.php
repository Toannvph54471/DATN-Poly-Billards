        ...
        <div class="bg-white shadow-md rounded-xl p-6 border border-gray-200">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-gray-700 flex items-center gap-2">
                    <i class="fa-solid fa-gift text-blue-600"></i> Chỉnh sửa chương trình khuyến mãi
                </h1>
            ...
            <form action="{{ route('admin.promotions.update', $promotion->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                        class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                        placeholder="Mô tả chi tiết chương trình...">{{ old('description', $promotion->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <select name="discount_type" id="discount_type"
                            ...
                            <option value="fixed"
                                {{ old('discount_type', $promotion->discount_type) === 'fixed' ? 'selected' : '' }}>Số tiền
                                cố định
                            </option>
                        </select>
                        @error('discount_type')
                    ...
                    <div>
                        ...
                        <input type="number" name="min_total_amount" min="0"
                            value="{{ old('min_total_amount', empty($promotion->min_total_amount) ? 100000 : $promotion->min_total_amount) }}"
                            class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="end_date" value="{{ old('end_date', $promotion->end_date) }}"
                            class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:outline-none"
                            required>
                        @error('end_date')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
